//All the business Data goes here
...
const businesses = [
  ...
  export const getNYbusinessList = () => {
    let nyBusinessArr = [];
    businesses.filter(busyObj => {
      if(busyObj.addressStateCode=== "NY"){
        nyBusinessArr.push(busyObj);
      }
    })
    return nyBusinessArr;
}

export const getManufacturingBusinessList = () => {
  let ManBusinessArr = [];
  businesses.filter(busyObj => {
    if(busyObj.companyIndustry == "Manufacturing"){
      ManBusinessArr.push(busyObj);
    }
  })
  return ManBusinessArr;
}
